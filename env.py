...
class Environment():
    ...
    def step(self, action=None, seed=None):
        ...
        return self.state.to_numpy(), self.reward, self.done, self.steps

    def render(self, Xlabel, Ylabel_1, Ylabel_2):
        # 可视化当前环境
        plt.plot(Xlabel, Ylabel_1, label = "PPO")
        plt.plot(Xlabel, Ylabel_2, label = "Random") 
        plt.legend(['PPO', 'Random'])
        plt.title('Rewards')
        plt.xlabel('Episode')
        plt.ylabel('Reward')
        plt.show()
